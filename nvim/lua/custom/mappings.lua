    ...
    n = {
        ["<leader>bf"] = {
            function()
                ...
                -- Only works inside Markdown files
                if vim.bo.filetype ~= "markdown" then
                    vim.notify("  This command only works in Markdown files!", vim.log.levels.ERROR)
                    return
                end

                -- 1️⃣ Enable spell check if not already
                if not vim.wo.spell then
                    vim.wo.spell = true
                end

                -- 2️⃣ Disable SuperMaven for this buffer
                vim.cmd("SupermavenStop")

                -- 3️⃣ Detach all LSP clients except thesaurus
                local clients = vim.lsp.get_active_clients()
                for _, client in ipairs(clients) do
                    if vim.lsp.buf_is_attached(buf, client.id) and client.name ~= "thesaurus" then
                        vim.lsp.buf_detach_client(buf, client.id)
                    end
                end

                -- Success notification
                vim.notify("  Markdown setup applied successfully!", vim.log.levels.INFO)
            end,
            "Setup Markdown tools (spell, LSP, plugins, etc.)",
        },
    },
}
...
M.snacks = {
    ...
    n = {
        ...
        ["<leader>bA"] = {
            function()
                vim.notify("Opening GitHub Account...", vim.log.levels.INFO, {
                    title = "GitHub",
                    icon = "",
                })
...
M.general = {
    n = {
        ...
        ["<leader>anf"] = {
            function()
                Snacks.picker.select(
                    { "File", "Directory" },
                    { prompt = "Create:", win = { width = 0.25, height = 0.30 } },
                    function(choice)
                        if not choice then
                            return
                        end
        ...
        ["<leader>it"] = { "<cmd> :InspectTree <CR>", "Open Inspect Tree" },
